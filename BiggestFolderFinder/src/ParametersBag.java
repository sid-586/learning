...
public class ParametersBag {
    ...
    public ParametersBag(String args[]) {
        ...
        if (args.length != 4) {
            throw new IllegalArgumentException("Укажите две параметра: -l (лимит по объему) и -d (путь к папке)");
        }
        limit = 0;
        path = "";
        for (int i = 0; i < 4; i += 2) {
            if (args[i].equals("-l")) {
                limit = ReadableSizeCalculator.getSizeFromHumanReadable(args[i + 1]);
